...
func (bf *batchFilter) batchParallel(count, batch int, cb func(offset, count int) error) error {
	...
	// If our Context timed out or was cancelled, forweard that error instead
...
// GetAllBatch is a batching version of GetAll. See RunBatch for more
// information about batching, and GetAll for more information about the
// parameters.
//
// If the Context supplied to GetAllBatch is cancelled or reaches its deadline,
// GetAllBatch will terminate with the Context's error.
//
// By default, datastore applies a short (~5s) timeout to queries. This can be
// increased, usually to around several minutes, by explicitly setting a
// deadline on the supplied Context.
//
// If the specified `batchSize` is <= 0, no batching will be performed.
func GetAllBatch(c context.Context, batchSize int32, q *Query, dst interface{}) error {
	return GetAll(withQueryBatching(c, batchSize), q, dst)
}
