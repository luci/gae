...
func reduce(fq *ds.FinalizedQuery, kc ds.KeyContext, isTxn bool) (*reducedQuery, error) {
	...
	if isTxn && fq.Original().GetFirestoreMode() && fq.EventuallyConsistent() {
		return nil, fmt.Errorf("queries within a transaction to firestore cannot be eventually consistent")
	}
