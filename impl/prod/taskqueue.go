...
func (t tqImpl) AddMulti(tasks []*tq.Task, queueName string, cb tq.RawTaskCB) error {
	for _, realTasks := range makeBatches(tqMF2R(tasks), constraints.TQ().MaxAddSize) {
...
func (t tqImpl) DeleteMulti(tasks []*tq.Task, queueName string, cb tq.RawCB) error {
	offset := 0
	for _, realTasks := range makeBatches(tqMF2R(tasks), constraints.TQ().MaxDeleteSize) {
		switch err := taskqueue.DeleteMulti(t.aeCtx, realTasks, queueName).(type) {
		case appengine.MultiError:
			for i, e := range err {
				if e != nil {
					cb(offset+i, e)
