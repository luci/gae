...
func (f *Flex) Configure(c context.Context, opts ...option.ClientOption) (cfg *Config, err error) {
	...
	withScopedTokenSource := func(scopes ...string) ([]option.ClientOption, error) {
		ts, err := gsp.TokenSource(c, scopes...)
		if err != nil {
			return nil, err
		}

		// Note that we re-use "opts" here. However, since the original "opts"
		// bounds haven't changed, this will reuse the same additional slots.
		return append(opts, option.WithTokenSource(ts)), nil
	}

	// Cloud Datastore Client
	clientOpts, err := withScopedTokenSource(datastore.ScopeDatastore)
	if err != nil {
		panic(errors.Annotate(err, "could not get datastore token source").Err())
	}
	if cfg.DS, err = datastore.NewClient(c, cfg.ProjectID, clientOpts...); err != nil {
		panic(errors.Annotate(err, "failed to instantiate datastore client").Err())
	}

	// Cloud Logging Client
	clientOpts, err = withScopedTokenSource(cloudLogging.WriteScope)
	if err != nil {
		panic(errors.Annotate(err, "could not get logging token source").Err())
	}
