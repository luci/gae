...
type Flex struct {
	// Cache is the process-global cache that Flex services can use.
	Cache ProcessCache
}

// Configure constructs a Config based on the current Flex environment.
//
// Probe will instantiate some cloud clients. It is the responsibility of the
// client to close those instances when finished.
//
...
func (f *Flex) Configure(c context.Context, opts ...option.ClientOption) (cfg *Config, err error) {
	...
	cfg.ServiceProvider = &GoogleServiceProvider{
		ServiceAccount: cfg.ServiceAccountName,
		Cache:          cache,
	}

	// Install our GCE access token provider.
...
// Request probes Request parameters from a AppEngine Flex Environment HTTPw
// request.
