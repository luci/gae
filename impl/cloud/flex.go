...
func (f *Flex) Configure(c context.Context, opts ...option.ClientOption) (cfg *Config, err error) {
	...
	if metadata.OnGCE() {
		...
		creds, err := google.FindDefaultCredentials(c, iamAPI.CloudPlatformScope)
		if err != nil {
			return nil, errors.Annotate(err, "failed to get Application Default Credentials").Err()
		}
		if cfg.ServiceAccountName, err = getEmailFromTokenSource(c, creds.TokenSource); err != nil {
			return nil, err
		}
	...
	if cfg.ProjectID, err = getEnv("GCLOUD_PROJECT", cfg.ProjectID); err != nil {
		return nil, err
	}
	if cfg.ServiceName, err = getEnv("GAE_SERVICE", cfg.ServiceName); err != nil {
		return nil, err
	}
	if cfg.VersionName, err = getEnv("GAE_VERSION", cfg.VersionName); err != nil {
		return nil, err
	}
	if cfg.InstanceID, err = getEnv("GAE_INSTANCE", cfg.InstanceID); err != nil {
		return nil, err
	}
...
func getEnv(key, def string) (string, error) {
	switch v := os.Getenv(key); {
	case v != "":
		return v, nil
	case def == "":
		return "", errors.Reason("missing environment variable %q", key).Err()
	default:
		return def, nil
	}
}
