...
func (cfg *Config) Use(c context.Context, req *Request) context.Context {
	...
	if cfg.L != nil {
		...
		l := Logger{cfg.L.Logger("request", clOpts...)}
		c = l.Use(c)
	} else {
	...
	if isb.cache == nil {
		// Always having a cache-like object simplifies info service code.
		isb.cache = nopCache{}
	}

	// Determine our access token source.
	//
	// Currently, we only support GCE access tokens.
	c = useInfo(c, &isb)
