...
// infoStateBase is the set of base, immutable info service values. These are
// initialized when the service is instantiated.
type infoStateBase struct {
	*Config
	*Request
...
// infoState is a reconstruction of the info service for the cloud API.
//
// It is bound to a global info state (infoStateBase), and includes
...
func useInfo(c context.Context, base *infoStateBase) context.Context {
...
func (i *infoService) AccessToken(scopes ...string) (token string, expiry time.Time, err error) {
	...
	if tok, err = i.ServiceProvider.AccessToken(i, scopes...); err != nil {
		return
	}
