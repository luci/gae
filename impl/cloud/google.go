...
func (gsp *GoogleServiceProvider) AccessToken(c context.Context, scopes ...string) (*oauth2.Token, error) {
	...
	// element.
	if tokIface, ok := gsp.Cache.Get(c, cacheKey); ok {
		tok := tokIface.(*oauth2.Token)
		if !closeToExpRandomized(c, now, tok.Expiry, accessTokenExpirationRandomization) {
			return tok, nil
		}
	}

	// Either the token is expired, or it was selected randomly as a refresh case.
	// Refresh the token.
	//
	...
	tokIface, err := gsp.Cache.GetOrCreate(c, cacheKey, func() (interface{}, time.Duration, error) {
...
func closeToExpRandomized(c context.Context, now, exp time.Time, expRandomization time.Duration) bool {
	switch {
	case now.After(exp):
