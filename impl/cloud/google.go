...
type GoogleServiceProvider struct {
	// ServiceContext is the service-level Context. This is used for global-level
	// operations, even if they may be executing on behalf of individual
	// operations, notably access token refresh.
...
func (gsp *GoogleServiceProvider) TokenSource(c context.Context, scopes ...string) (oauth2.TokenSource, error) {
	...
	sort.Strings(scopes)
	cacheKey := googleTokenSourceKey(strings.Join(scopes, "\x00"))

	// Get our cached TokenSource or, if it's not available, create a new one. A
	// generated TokenSource has no cache expiration.
	//
	// While refreshing, we lock around the cache key via GetOrCreate to serialize
	// individual requests.
	tsIface, err := gsp.Cache.GetOrCreate(c, cacheKey, func() (interface{}, time.Duration, error) {
		// Obtain a new DefaultTokenSource bound to our service-level Context. The
		// TokenSource may retain the Context and use it to refresh tokens, and we
		// don't want it to be subjected to whatever constraints this individual
		// TokenSource request's Context has.
		ts, err := google.DefaultTokenSource(gsp.ServiceContext, scopes...)
		if err != nil {
			return nil, 0, err
		}
		return ts, 0, nil
	...
	return tsIface.(oauth2.TokenSource), nil
}
