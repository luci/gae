...
// Logger is an implementation of logging.Logger that is bound to a StackDriver
// Logger.
//
// A Logger can be installed into a Context with its Use method.
type Logger struct {
	// Client is the underlying Cloud Logging Client to use.
	CloudLogger *cloudLogging.Logger
}

// Use installs an instance of this Logger into the supplied Context.
func (l *Logger) Use(c context.Context) context.Context {
...
func (bl *boundLogger) LogCall(lvl logging.Level, calldepth int, format string, args []interface{}) {
	...
	bl.cl.CloudLogger.Log(cloudLogging.Entry{
